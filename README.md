# Family Office Portfolio Analysis

## Project Overview

This project involved evaluating an existing Family Office equity portfolio, identifying underperforming holdings, constructing a diversified investment portfolio, and tracking performance over a two-week period to validate investment recommendations.

### Project Objective

To identify high-quality investment opportunities through fundamental and technical analysis, optimize portfolio allocation, and evaluate portfolio performance using a structured investment framework.

---
## Business Impact

- Evaluated 5 existing equity holdings using fundamental and technical analysis.
- Identified portfolio strengths, weaknesses, and potential risk exposures.
- Constructed a diversified portfolio comprising 10 stocks across multiple sectors.
- Applied asset allocation principles to balance risk and return.
- Validated recommendations through a two-week performance monitoring exercise.

---

## Key Outcome

📈 Constructed and monitored a diversified ₹10 lakh virtual equity portfolio that generated a return of **+2.6%** during a two-week evaluation period where Nifty 50 and Sensex grew by **+0.26%** and **-0.07%** resp.

---

## Project Workflow

![Project Workflow](/Images/Work%20Flow.png)

### 1. Portfolio Evaluation

Analysis of the existing portfolio using:

- Fundamental Analysis
- Technical Analysis
- Investment Recommendations

📂 [View Component 1](Component-1_Portfolio_Evaluation/README.md)

---

### 2. Portfolio Construction

Built a diversified ₹10 lakh virtual portfolio using:

- Market Research
- Stock Screening
- Asset Allocation

📂 [View Component 2](Component-2_Portfolio_Construction/README.md)

---

### 3. Performance Tracking

Monitored portfolio performance over a two-week period and evaluated returns.

📂 [View Component 3](Component-3_Performance_Tracking/README.md)

---

## Portfolio Allocation

![Portfolio Allocation](/Images/Screenshot%202026-06-13%20022722.png)



---

## Fundamental Analysis Snapshot

![Fundamental Analysis](Images/Screenshot%202026-06-13%20021711.png)

---

## Portfolio Performance

![Portfolio Allocation](/Images/Screenshot%202026-06-13%20023157.png)

---

## Skills Demonstrated

- Equity Research
- Fundamental Analysis
- Technical Analysis
- Portfolio Construction
- Asset Allocation
- Risk Management
- Financial Analysis
- Investment Strategy
- Business Reporting

---

## Tools Used

- Microsoft Excel
- Annual Reports
- Financial Statements
- Market Data Sources

## Analytical Techniques

- Fundamental Analysis
- Technical Analysis
- Ratio Analysis
- Portfolio Allocation
- Investment Evaluation

---

## Portfolio Stocks

### Existing Portfolio Evaluated

- HUL
- Bharat Electronics
- Tata Steel
- NTPC
- ONGC

### New Portfolio Constructed

- TCS
- HDFC Bank
- Nestlé India
- Tata Power
- Larsen & Toubro
- Dr. Reddy's Laboratories
- Mahindra & Mahindra
- Bharti Airtel
- Avenue Supermarts
- SBI Life Insurance

---

## Author

**Ayush Anand**

MBA (Analytics) '28, IIM Kashipur

Ex-Marketing Executive

Business Analytics | Finance | Strategy
